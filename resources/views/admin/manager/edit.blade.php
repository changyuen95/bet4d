                ...
                <div class="p-6">
                    ...
                    <form action="{{ route('admin.managers.update', $manager->id) }}" method="POST">
                        ...
                        <div class="mb-6">
                            ...
                            <input type="text" id="name" name="name" value="{{ old('name', $manager->name) }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required>
                        </div>
                        ...
                        <div class="mb-6">
                            ...
                            <input type="text" id="ic" name="ic" value="{{ old('ic', $manager->formatted_ic) }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" placeholder="123456781234 or 123456-78-1234" required>
                            <p class="mt-1 text-sm text-gray-500">Enter IC without spaces or with format: xxxxxx-xx-xxxx</p>
                        </div>

                        <div class="mb-6">
                            <label for="phone" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Phone Number</label>
                            <input type="text" id="phone" name="phone" value="{{ old('phone', $manager->phone) }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                        </div>
                        ...
                        <div class="mb-6">
                            ...
                            <select id="role" name="role" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required>
                                ...
                                <option value="1" {{ old('role', $manager->role) == '1' ? 'selected' : '' }}>Recorder (Recorded By)</option>
                                <option value="2" {{ old('role', $manager->role) == '2' ? 'selected' : '' }}>Certifier (Certified By)</option>
                                <option value="3" {{ old('role', $manager->role) == '3' ? 'selected' : '' }}>Both</option>
                            </select>
                        </div>
                        ...
                        <div class="mb-6">
                            ...
                            <select id="is_active" name="is_active" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required>
                                <option value="1" {{ old('is_active', $manager->is_active) == '1' ? 'selected' : '' }}>Active</option>
                                <option value="0" {{ old('is_active', $manager->is_active) == '0' ? 'selected' : '' }}>Inactive</option>
                            </select>
                        </div>

                        <div class="mb-6">
                            <label for="remarks" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Remarks</label>
                            <textarea id="remarks" name="remarks" rows="2" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">{{ old('remarks', $manager->remarks) }}</textarea>
                        </div>

                        <div class="flex gap-2">
                            <button type="submit" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5">Update Manager</button>
