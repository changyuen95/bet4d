    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <div class="flex gap-2">
                <a href="{{route('admin.managers.create')}}" class="focus:outline-none text-white bg-purple-700 hover:bg-purple-800 focus:ring-4 focus:ring-purple-300 font-medium rounded-lg text-md px-4 py-2 dark:bg-purple-600 dark:hover:bg-purple-700 dark:focus:ring-purple-900">Create Manager</a>
            </div>
        </div>
    </x-slot>
    ...
    <div class="py-4">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg mb-4">
                <div class="p-6">
                    <form method="GET" action="{{ route('admin.managers.index') }}" class="flex flex-wrap gap-4">
                        ...
                        <div class="flex-1 min-w-[200px]">
                            ...
                            <input type="text" id="ic" name="ic" value="{{ request('ic') }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" placeholder="xxxxxx-xx-xxxx">
                        </div>
                        <div class="flex-1 min-w-[200px]">
                            ...
                            <select id="role" name="role" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                                ...
                                <option value="1" {{ request('role') == '1' ? 'selected' : '' }}>Recorder</option>
                                <option value="2" {{ request('role') == '2' ? 'selected' : '' }}>Certifier</option>
                                <option value="3" {{ request('role') == '3' ? 'selected' : '' }}>Both</option>
                            </select>
                        </div>
                        <div class="flex-1 min-w-[200px]">
                        ...
                        <div class="flex items-end gap-2">
                            ...
                            <a href="{{ route('admin.managers.index') }}" class="text-white bg-gray-700 hover:bg-gray-800 focus:ring-4 focus:ring-gray-300 font-medium rounded-lg text-sm px-5 py-2.5">Reset</a>
                        </div>
                    </form>
                </div>
            </div>
                ...
                <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-800">
                    <thead class="text-xs text-gray-700 uppercase bg-cyan-100 dark:bg-cyan-100 dark:text-gray-900">
                        <tr>
                            ...
                            <th scope="col" class="text-base px-6 py-4">Status</th>
                    ...
                    <tbody>
                        ...
                        <tr class="odd:bg-white odd:dark:bg-gray-100 even:bg-gray-50 even:dark:bg-gray-200 border-b dark:border-gray-700">
                            <td class="px-6 py-4">{{ $managers->firstItem() + $index }}</td>
                            <td class="px-6 py-4 font-medium text-gray-900">{{ $manager->name }}</td>
                            <td class="px-6 py-4">{{ $manager->formatted_ic }}</td>
                            ...
                            <td class="px-6 py-4">
                                <div class="flex gap-2">
                                    <a href="{{ route('admin.managers.show', $manager->id) }}" class="font-medium text-blue-600 hover:underline">View</a>
                                    <a href="{{ route('admin.managers.edit', $manager->id) }}" class="font-medium text-green-600 hover:underline">Edit</a>
                                    <form action="{{ route('admin.managers.destroy', $manager->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this manager?');">
                                        @csrf
                                        @method('DELETE')
                        ...
                        @empty
                        <tr>
                            <td colspan="7" class="px-6 py-4 text-center text-gray-500">No managers found.</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
